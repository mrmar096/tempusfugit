    ...
    <?php foreach($data as $apt) { ?>
        <tr id="table_tr_<?=$apt->id?>">
            ...
            <td>
                ...
                <a href="<?=base_url('admin/deleteapt/')?>" class="btn  btn-xs btn-raised  btn-danger" onclick="return deleteObj(this.href,<?=$apt->id?>,'table_tr_<?=$apt->id?>');"><i class="material-icons">delete</i> </a>
            </td>
        </tr>
        <?php
    }
